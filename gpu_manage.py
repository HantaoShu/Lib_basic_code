...
serverlist=['10.0.0.2', '10.0.0.3', '10.0.0.4', '10.0.0.12', '10.0.0.5','10.0.0.6','10.0.0.7','10.0.0.8','10.0.0.10']
def check_gpus():
    # =============================================================================
...
def query_gpu(qargs=[],server=''):
    ...
    cmd = server+' nvidia-smi --query-gpu={} --format=csv,noheader'.format(','.join(qargs))
    try:
        results = os.popen(cmd).readlines()
    except:
        results = []
    return [parse(line, qargs,server) for line in results]

def find_all_gpus(serverlist=serverlist,min_memory=4000):
    res = []
    for server in serverlist:
        tmp = query_gpu(server='ssh ' + server)
        res += tmp
    res = pd.DataFrame(res)
    res.dropna(inplace=True)
    res = res[res['memory.free']>min_memory]
    res['server'] = res['server'].apply(lambda x:x.split(' ')[1])
    print(res)
    res = res.sort_values('memory.free')
    return res

def parse(line, qargs,server=''):
    '''
    line:
        a line of text
    qargs:
    ...
    output = {k: process(k, v) for k, v in zip(qargs, line.strip().split(','))}
...
if check_gpus():
    ...
    class GPUManager():
        ...
        def __init__(self, qargs=[],gpu_id=0):
            '''
            '''
            self.qargs = qargs
        ...
        def wait_until_free(self,free_memory):
            gpu_info = query_gpu()
            gpu_info = pd.DataFrame(gpu_info)
            gpu_info = gpu_info[gpu_info['index']==str(self.gpu_id)]
            while (gpu_info['memory.free']<free_memory).sum() ==1:
                gpu_info = query_gpu()
                gpu_info = pd.DataFrame(gpu_info)
                gpu_info = gpu_info[gpu_info['index']==str(self.gpu_id)]
                print(gpu_info)
    ...
    print('GPU available check failed')
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--min_memory', type=int, default=4000)
    args = parser.parse_args()
    find_all_gpus(min_memory=args.min_memory)